...
for i in os.listdir():
    if not ".exe" in i:
        ...
        if len(info.extension) > 0:
            if not os.path.exists(os.getcwd() + "\\" + info.extension[-1]):
                os.mkdir(info.extension[-1])
            shutil.move(os.getcwd() + "\\" + i, os.getcwd() + "\\"+ info.extension[-1] +"\\" + i+"."+info.extension[-1])
        else:
            if not os.path.exists(os.getcwd() + "\\Unknown"):
                os.mkdir("Unknown")
            shutil.move(os.getcwd() + "\\" + i, os.getcwd() + "\\Unknown\\" + i)