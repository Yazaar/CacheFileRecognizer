    ...
    for i in files:
        if not "." in i:
            ...
            if len(info.extension) > 0:
                ...
                while root.split("\\")[-1] + str(antiCopy) + "-" + i + "." + info.extension[-1] in os.listdir(pathway + "\\" + info.extension[-1]):
                    antiCopy += 1
                shutil.move(root + "\\" +i, pathway + "\\" + info.extension[-1] + "\\" + root.split("\\")[-1] + str(antiCopy) + "-" + i + "." + info.extension[-1])
            else:
                if not os.path.exists(pathway + "\\Unknown"):
                    os.mkdir(pathway + "\\Unknown")
                while root.split("\\")[-1] + str(antiCopy) + "-" + i in os.listdir(pathway + "\\Unknown"):
                    antiCopy += 1
                shutil.move(root + "\\" +i, pathway + "\\Unknown\\" + root.split("\\")[-1] + str(antiCopy) + "-" + i)
